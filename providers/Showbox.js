...
const getCookieForRequest = async (regionPreference = null, userCookie = null) => {
    ...
    const originalRegion = regionPreference;
    let usingFallback = false;
    let baseCookieToUse = null;

    // Region detection
    if (regionPreference) {
        if (global.regionAvailabilityStatus && global.regionAvailabilityStatus[regionPreference] === false) {
            usingFallback = true;
            for (const fallback of US_FALLBACK_REGIONS) {
                if (!global.regionAvailabilityStatus || global.regionAvailabilityStatus[fallback] !== false) {
                    detectedOssGroup = fallback;
                    console.log(`[CookieManager] Region ${regionPreference} unavailable, fallback to ${detectedOssGroup}`);
                    break;
                }
            }
            if (!detectedOssGroup) {
                detectedOssGroup = regionPreference;
                console.log(`[CookieManager] All fallbacks unavailable, trying original ${detectedOssGroup}`);
            }
        } else {
            detectedOssGroup = regionPreference;
            console.log(`[CookieManager] Using explicit region: ${detectedOssGroup}`);
        }
    } else {
    ...
    if (baseCookieToUse && baseCookieToUse.trim() !== '') {
        ...
        if (detectedOssGroup) return `oss_group=${detectedOssGroup}`;
        return '';
    }
};
...
const CACHE_DIR = process.env.SHOWBOX_CACHE_DIR
...
const getFromCache = async (cacheKey, subDir = '') => {
    ...
    // File system cache only (Redis removed)
    ...
    try {
        ...
        // Redis removed: no cross-population
        try {
...
const _searchAndExtractShowboxUrl = async (searchTerm, originalTmdbTitle, mediaYear, showboxScraperInstance, tmdbType, regionPreference = null, tmdbAllTitles = []) => {
    ...
    // mediaTypeString removed (was unused after scoring refactor)
    ...
    const specialTitlesFromAll = tmdbAllTitles.filter(title =>
        ...
        (title.length > originalTmdbTitle.length ||
            /[\u0080-\uFFFF]/.test(originalTmdbTitle))
    ).slice(0, 2);
...
const getShowboxUrlFromTmdbInfo = async (tmdbType, tmdbId, regionPreference = null) => {
    ...
    if (tmdbType === 'movie') {
        ...
        const rawFirstAirDate = tmdbData.first_air_date;
...
const fetchSourcesForSingleFid = async (fidToProcess, shareKey, regionPreference = null, userCookie = null, retryAttempt = 0) => {
    ...
    const postDataForAxios = targetPostData.toString();
    const axiosConfig = { headers: baseHeaders, timeout: 20000 };
    ...
    async extractFebboxShareLinks(showboxUrl) {
        ...
        try {
            ...
            if (!contentInfo || !contentInfo.id || !contentInfo.type) {
                ...
                // console.timeEnd removed (timerLabel eliminated)
                return [];
            }
...
const extractFidsFromFebboxPage = async (febboxUrl, regionPreference = null, userCookie = null) => {
    // Removed timerLabel profiling label from extractFidsFromFebboxPage
    ...
    if (cachedParsedData && typeof cachedParsedData === 'object') { // Ensure it's an object
        ...
        // profiling disabled (timerLabel removed)
...
// Public API: fetch streams by TMDB reference
...
const processShowWithSeasonsEpisodes = async (febboxUrl, showboxTitle, seasonNum, episodeNum, resolveFids = true, regionPreference = null, userCookie = null, retryAttempt = 0) => {
    ...
    if (!contentHtml) {
        ...
        const finalFebboxUrl = febboxUrl;
        const axiosConfigMainPage = {
    ...
    let folderHtml = null;
    const episodeDetails = []; // Declare episodeDetails here, initialized as an empty array
    const episodeFids = []; // Declare episodeFids here, initialized as an empty array
    ...
    if (cachedParsedEpisodeList && Array.isArray(cachedParsedEpisodeList)) {
        ...
        if (!folderHtml) {
            ...
            try {
                ...
                const finalFolderUrl = targetFolderListUrl;
                const axiosConfigFolder = {
                    headers: {
...
const sortStreamsByQuality = (streams) => {
    // Quality ranking for output ordering
    const qualityOrder = {
...
// Fallback cookie loader removed (legacy loadFallbackCookies was deprecated).
// No-op now: runtime only uses user-supplied cookies from config/override.

// PStream API Configuration
...
// Function to map FebBox region to PStream region
const mapRegionForPStream = (febboxRegion) => {
    if (!febboxRegion) {
...
// Function to fetch streams from PStream API (new primary method)
const getStreamsFromPStreamAPI = async (imdbId, tmdbType, seasonNum = null, episodeNum = null, regionPreference = null, userCookie = null) => {
    ...
    try {
        ...
        let apiUrl;
        if (tmdbType === 'movie') {
            apiUrl = `${PSTREAM_API_BASE_URL}/movie/${imdbId}`;
        } else if (tmdbType === 'tv' && seasonNum !== null && episodeNum !== null) {
            apiUrl = `${PSTREAM_API_BASE_URL}/tv/${imdbId}/${seasonNum}/${episodeNum}`;
        } else {
        ...
        const headers = {
            ...
            'referer': `https://pstream.mov/media/tmdb-${tmdbType}-${imdbId}`,
            'region': pstreamRegion,
        ...
        if (uiToken) {
            headers['ui-token'] = uiToken;
            console.log(`[PStream] Using ui-token for authentication`);
        } else {
...
const processPStreamResponse = (apiResponse, tmdbType, seasonNum, episodeNum) => {
    ...
    try {
        ...
        if (apiResponse && apiResponse.message) {
            console.log(`[PStream] API returned message:`, apiResponse.message);
...
// Function to get TMDB data specifically for PStream API (extracts IMDB ID)
const getTmdbDataForPStream = async (tmdbType, tmdbId) => {
    ...
    if (!tmdbData || process.env.DISABLE_CACHE === 'true') {
        ...
        try {
            ...
            console.log(`  [PStream] Error fetching TMDB data: ${error.message}`);
            return null;
        }
    }
    ...
    if (!externalIdsData || process.env.DISABLE_CACHE === 'true') {
        ...
        try {
            ...
            if (externalIdsData) {
                if (process.env.DISABLE_CACHE !== 'true') {
                    await saveToCache(externalIdsCacheKey, externalIdsData, cacheSubDir);
                }
            }
        } catch (error) {
...
// Function to get ui-token for PStream API (handles JWT tokens)
const getUiTokenForPStream = async (userCookie = null) => {
    // If user provided a JWT token directly, use it
