...
function getUhdMoviesKey() {
  const k = getTmdbApiKey();
  if (!k) console.warn('[UHDMovies] TMDB API key missing; certain metadata features skipped.');
  return k;
}
...
// Caching layer simplified: placeholders only (extend for FS persistence if needed)
...
const saveToCache = async (key, data) => {
  if (!CACHE_ENABLED) return;
  // no-op (placeholder for future FS caching)
};
...
async function extractTvShowDownloadLinks(showPageUrl, season, episode) {
  try {
    ...
    if (downloadLinks.length === 0) {
      ...
      const actualSeasonsOnPage = new Set(); // Track what seasons actually have content
...
async function getUHDMoviesStreams(tmdbId, mediaType = 'movie', season = null, episode = null) {
  ...
  try {
    ...
    if (cachedLinks && cachedLinks.length > 0) {
      ...
      const TMDB_API_KEY_UHDMOVIES = getUhdMoviesKey();
      ...
      const searchTitle = mediaInfo.title.replace(/:/g, '').replace(/\s*&\s*/g, ' and ');
