        ...
        $('script').each((i, script) => {
            ...
            if (scriptContent) {
                ...
                for (const regex of sourcesRegexes) {
                    ...
                    if (regex.source.includes('file') && regex.source.includes('sources')) { // Heuristic to identify complex sources regex capturing group 2
...
if (require.main === module) {
        main()
            .then(code => {
                if (code !== 0) {
                    console.error(`[vidsrcextractor] Finished with non-zero code ${code}`);
                }
            })
            .catch(error => {
